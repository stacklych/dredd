...
hooks.after(
  '/resource > Update Resource > 200 > text/plain; charset=utf-8',
  (transaction, done) => {
    const replaceToken = (body) =>
      body.replace(tokenPattern, '--- CENSORED ---');

    // Remove sensitive data from Dredd transaction
    transaction.test.actual.body = replaceToken(transaction.test.actual.body);
    transaction.test.expected.body = replaceToken(
